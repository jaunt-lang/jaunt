...
static private boolean isLeanVarByPredicate(Var var) {
    if (!LEAN_VAR_PRED.isBound())
        return true;
    IFn pred = (IFn)LEAN_VAR_PRED.deref();
    if (pred == null)
        return true;
    return RT.booleanCast(pred.invoke(var));
}
...
//			else
	...
	public void emit(C context, ObjExpr objx, GeneratorAdapter gen){
		...
		if(initProvided) {
		    try {
				Associative m = RT.map();
				if(isStrict)
					m = RT.assoc(m, STRICT_TAGS, isStrict);
				Var.pushThreadBindings(m);
...
//					                    " refers to:" + v);
			...
			registerVar(v, (leanCompile && isLeanVar(v)));
...
//					.without(Keyword.intern(null, "static"));
			...
			try {
				...
				Var.pushThreadBindings(RT.map(IS_ANALYZING_META, leanCompile && isLeanVar(v)));
				meta = mm.count()==0 ? null:analyze(context == C.EVAL ? context : C.EXPRESSION, mm);
			} finally {
				Var.popThreadBindings();
			}
			try {
				Associative m = RT.map(IS_DEFINING_LEAN_VAR, leanCompile && isLeanVar(v),
...
//						throw Util.runtimeException("Non-static fn can't have primitive parameter: " + p);
	...
	private IPersistentMap initStrictLocals(ObjExpr fn, GeneratorAdapter gen) {
		...
		if(strictMode()) {
			...
			for (ISeq lbs = argLocals.seq(); lbs != null; lbs = lbs.next()) {
				...
				if(t == null || OBJECT_TYPE.equals(t)) {
					t = Type.getType(tagClass(lb.tag));
				}

				if(t != Type.LONG_TYPE
								&& t != Type.DOUBLE_TYPE
								&& !OBJECT_TYPE.equals(t)) {
					gen.loadArg(lb.idx-argoff);
					gen.checkCast(t);
					gen.storeArg(lb.idx-argoff);
				}

				ret = ret.assoc(lb, gen.mark());
			}
		} else {
			final Label start = gen.mark();
			for (ISeq lbs = argLocals.seq(); lbs != null; lbs = lbs.next())
				ret = ret.assoc(lbs.first(), start);
		}
		return ret;
	}
	...
	public void doEmitStatic(ObjExpr fn, ClassVisitor cv){
		...
		GeneratorAdapter gen = new GeneratorAdapter(ACC_PUBLIC + ACC_STATIC,
		                                            ...
		                                            cv);
		gen.visitCode();
		IPersistentMap startPoints = initStrictLocals(fn, gen);
		...
		try {
			...
			for(ISeq lbs = argLocals.seq(); lbs != null; lbs = lbs.next()) {
				LocalBinding lb = (LocalBinding) lbs.first();
				Type t = argtypes[lb.idx];
				if(strictMode() && (t == null || OBJECT_TYPE.equals(t))) {
					t = Type.getType(tagClass(lb.tag));
				}
				gen.visitLocalVariable(lb.name, t.getDescriptor(), null, (Label)startPoints.valAt(lb), end, lb.idx);
			}
		} finally {
			Var.popThreadBindings();
		}
	...
	public void doEmitPrim(ObjExpr fn, ClassVisitor cv){
		...
		Type[] realArgs = new Type[argtypes.length];
		for (int i = 0; i < argtypes.length; i++) {
			final Type t = argtypes[i];
			realArgs[i] = (t == Type.LONG_TYPE || t == Type.DOUBLE_TYPE) ? t : OBJECT_TYPE;
		}
		Method ms = new Method("invokePrim", returnType, realArgs);
	...
	public void doEmit(ObjExpr fn, ClassVisitor cv){
		...
		GeneratorAdapter gen = new GeneratorAdapter(ACC_PUBLIC,
		                                            ...
		                                            cv);
		gen.visitCode();
		final IPersistentMap startPoints = initStrictLocals(fn, gen);
		...
		try
			...
			for(ISeq lbs = argLocals.seq(); lbs != null; lbs = lbs.next())
				...
				gen.visitLocalVariable(lb.name, t, null, (Label)startPoints.valAt(lb), end, lb.idx);
				}
			}
		finally
			{
...
static Var lookupVar(Symbol sym, boolean internNew) {
	return lookupVar(sym, internNew, !RT.booleanCast(LEAN_COMPILE.deref()));
}

static Var lookupVar(Symbol sym) {
	return lookupVar(sym, true, !RT.booleanCast(LEAN_COMPILE.deref()));
}
...
private static void registerVar(Var var, boolean topLevel) {
	...
	if(id == null)
		{
		id = registerConstant(var, RT.booleanCast(LEAN_COMPILE.deref()) && isLeanVar(var));
...
//		ret.volatiles = PersistentHashSet.create(RT.seq(RT.get(ret.optionsMap, volatileKey)));
		...
		try {
				Associative binds =
					RT.mapUniqueKeys(CONSTANTS, PersistentVector.EMPTY,
									 ...
									 NO_RECUR, null);
				if(strict)
					binds = RT.assoc(binds, STRICT_TAGS, strict);
				Var.pushThreadBindings(binds);
		...
		try {
			Associative m = RT.map(EMIT_LEAN_CODE, false);
			if(strict)
				m = RT.assoc(m, STRICT_TAGS, strict);
			Var.pushThreadBindings(m);
